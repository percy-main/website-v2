#!/usr/bin/env bash
# bastion-sql.sh — Spin up a short-lived EC2 bastion, open an SSH tunnel
# to RDS, run a SQL command, then tear everything down.
#
# Usage:
#   ./scripts/bastion-sql.sh "UPDATE player_sponsorship SET slug = 'alex-young' WHERE player_name = 'Alex Young';"
#
# Prerequisites:
...
if [[ ${#SQL_COMMANDS[@]} -eq 0 ]]; then
  echo "Usage: ./scripts/bastion-sql.sh \"SQL COMMAND\" [\"SQL COMMAND 2\" ...]"
  echo ""
  echo "Each argument is executed as a separate SQL statement against production RDS."
  exit 1
fi
...
SG_ID=$(aws --profile "$PROFILE" --region "$REGION" ec2 create-security-group \
  ...
  --description "Temporary bastion for SQL" \
  --vpc-id "$VPC_ID" \
...
INSTANCE_ID=$(aws --profile "$PROFILE" --region "$REGION" ec2 run-instances \
  ...
  --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=bastion-sql-tmp}]" \
...
for cmd in "${SQL_COMMANDS[@]}"; do
  echo "> $cmd"
  PGPASSWORD="$RDS_PASS" psql -h localhost -p "$LOCAL_PORT" -U "$RDS_USER" -d "$RDS_DBNAME" \
    --set=sslmode=require -c "$cmd"
  echo ""
done
echo "======================================"
...
echo ""
echo "SQL commands complete. Cleaning up resources..."
