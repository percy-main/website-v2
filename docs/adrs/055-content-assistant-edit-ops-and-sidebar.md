...
4. **The client applies ops best-effort with stale-id guards.** The user may edit (or restore a revision) mid-stream, so `applyEditOps` skips ops whose target id no longer resolves rather than throwing - the draft is the user's, the agent's view is advisory.
