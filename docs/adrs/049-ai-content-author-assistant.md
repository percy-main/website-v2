...
1. **Reuse, don't re-mode.** `createContentAuthorAgent` imports `createPlayCricketTools`, `createDbTools`, `createWeatherTools`, `createScoutCache`, `resolveModel` from `../scout/*` directly. Scout is not modified beyond one allowlist entry.
