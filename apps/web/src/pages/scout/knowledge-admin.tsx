...
/**
 * The 3-step upload flow, at module scope: the React Compiler can't
 * lower try/catch-with-finally (or throw / value blocks inside a try),
 * so keeping this inside UploadForm made the whole component bail out
 * of compilation. Module-level functions aren't compile candidates.
 */
async function uploadDocument(
  form: UploadFormState & { file: File },
  dispatch: (action: UploadFormAction) => void,
  onUploaded: () => void,
): Promise<void> {
  const { file, title, description, tagsRaw } = form;
...
function UploadForm({ onUploaded }: UploadFormProps) {
  ...
  const submit = async () => {
    if (!file) return;
    await uploadDocument({ ...form, file }, dispatch, onUploaded);
  };
