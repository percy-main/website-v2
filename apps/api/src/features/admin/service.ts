...
export async function listUsers(params: ListUsers) {
...
export async function sendChargeNotification(userId: string) {
...
export async function getRecordLinking() {
...
export async function linkPlayCricketPlayer(
...
export async function unlinkPlayCricketPlayer(
  type: Unlink["type"],
  id: string,
) {
  const table = type === "member" ? "member" : "dependent";
...
export async function linkContentfulPerson(
  memberId: string,
...
export async function unlinkContentfulPerson(memberId: string) {
  await client
    .updateTable("member")
