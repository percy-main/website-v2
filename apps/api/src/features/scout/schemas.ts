...
// retrieves chunks from via knowledge_search. Reads are gated by
// ai_knowledge.view, writes by ai_knowledge.manage. Uploads use the same
// three-step flow as attachments (mint → PUT → commit) but the worker
// that ingests is one-shot ECS, not in the API request lifecycle.
