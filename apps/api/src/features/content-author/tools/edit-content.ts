...
- { "op": "update", "blockId": "<id>", "block": { "type": ..., "props": ..., "content": ... } } - replace that block's type, props and content wholesale. Its nested child blocks are kept. Your content is plain text, so any bold/italic/links inside the old block are LOST - avoid updating blocks marked [has formatting] in the draft listing unless the user asked for that block to be rewritten. Updating a table replaces its whole content (column widths and cell styling reset).
