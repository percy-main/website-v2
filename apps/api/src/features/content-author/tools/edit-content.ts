...
- { "op": "update", "blockId": "<id>", "block": { "type": ..., "props": ..., "content": ... } } - rewrite that block. The replacement block is validated in full, same rules as write_content, so include every required prop (echo current values from the draft listing where you don't mean to change them). For TEXT blocks (paragraph, heading, list items, quote) you may omit "content": the block's existing text, formatting included, is kept - so a props-only update (e.g. changing a heading level) never destroys formatting. Nested child blocks are always kept. Providing "content" writes plain text: any bold/italic/links previously inside that block are LOST - avoid rewriting the content of blocks marked [has formatting] unless the user asked for that. A table update must re-specify the whole tableContent; cell styling resets - echo back columnWidths to keep the layout.
