 ...
 */
async function findOrCreateMember(email: string): Promise<string> {
 ...
 */
async function countDependentsThisYear(memberId: string): Promise<number> {
 ...
 */
export async function addDependents(
  email: string,
  dependents: DependentInput[],
) {
  const memberId = await findOrCreateMember(email);
  const existingCount = await countDependentsThisYear(memberId);
 ...
 */
export async function getDependents(email: string) {
 ...
 */
export async function listMyTeams(userId: string, role: string) {
  if (role === "admin") {
    return client
...
export async function listPlayers(
  ...
  const allDependents = await client
    .selectFrom("dependent")
...
export async function getPlayerDetail(
  ...
  if (role !== "admin") {
    ...
    const assignedTeam = await client
      .selectFrom("junior_team_manager")
      .innerJoin(
        "junior_team",
        "junior_team.id",
