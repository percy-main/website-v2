...
export async function getEligiblePlayers(season?: string) {
  ...
  const previousPoints = await client
  ...
  const teamCountResult = await client
...
export async function getMyTeam(userId: string, season?: string) {
  const s = season ?? getCurrentSeason();
  const gameweek = getCurrentGameweek(s);
...
export async function toggleEligibility(
...
export async function populatePlayers() {
...
export async function calculateSandwichCosts(season?: string) {
  const s = season ?? getCurrentSeason();
  const previousSeason = getPreviousSeason(s);
