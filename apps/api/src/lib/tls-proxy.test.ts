...
describe("startTlsProxy", () => {
  ...
  it("forwards TLS traffic and retains the valid context when refresh fails", async () => {
    ...
    const load = vi.fn().mockResolvedValueOnce(bundle).mockRejectedValue(new Error("renewal unavailable"));
    const proxy = await startTlsProxy({
      certificateArn: "test", passphrase: "test-passphrase", port: 0,
      upstreamPort, logger, refreshIntervalMs: 10, exportCertificate: load,
    });
    const proxyPort = (proxy.address() as { port: number }).port;

    const request = () => new Promise<string>((resolve, reject) => {
      const socket = connectTls({ port: proxyPort, rejectUnauthorized: false }, () => {
        socket.write("GET /health/ready HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n");
      });
      let data = "";
